...
$('div').live('pagecreate',function(event, ui) {
    $(this).hasClass('add-footer') && $(this).append('<div data-role="footer" data-position="fixed" id="footerID" data-tap-toggle="false">');
});
...
$('#settings').live('pagebeforeshow', function(event, ui) {
    flurryToggle();
    $('#clearData').bind('click', function (event, ui){
        clearData();
    });
});
...
$('.anger_assessment').live('pagecreate', function(event, ui) {
    /* Load results data */
    $.mobile.loadPage( "../modules/anger/assessment.js" ); 
    console.log('LA Page show call');
});
...
function mainFooter(){
    $('#footerID').html('<div data-role="navbar" >' + 
                        '<ul>' +
                        /*'<li><a href="about.html" data-rel="dialog"  data-theme="z" data-icon="info">About</a></li>' +*/
                        /*'<li><a href="#" data-theme="z" data-icon="grid" id="allToolsDialog">Tools</a></li>' + */
...
function alertDialog(pageText, headerText, back) {
    if (headerText == null) {
        ...
        //console.log('PAGE header: ' + pageText);
    }
    if(back == null || back == 0) {
        back = 1;
        //console.log('PAGE TEXT2: ' + pageText);
    }
    else {
        back = Math.abs(back);
        //console.log('PAGE TEXT3: ' + pageText);
    }
    
    $.mobile.changePage( '../../dialog.html' , { role: 'dialog'});

    $('#dialog').live('pagecreate', function(event, ui){          
